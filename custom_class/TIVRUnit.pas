...
begin
  ...
  try
    ...
    begin
      ...
      for j := 0 to 2 do
      begin
        Line := Line + p_ListView.Items[i].SubItems[j] + #9; // Используем табуляцию как разделитель
      end;
      // Добавляем основной текст элемента
      Line := p_ListView.Items[i].Caption + #9 + Line;
      // Удаляем последний разделитель
      if Line.EndsWith(#9) then
        SetLength(Line, Length(Line) - 1);
      Line := Line + sLineBreak; // Добавляем перевод строки
